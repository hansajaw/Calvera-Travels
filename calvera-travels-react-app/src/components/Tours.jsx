...
const BookingFormPopup = ({ pkg, onSubmit, onClose }) => {
  const [formData, setFormData] = useState({
    name: "", email: "", phone: "", date: "", travellers: "", note: ""
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/tour/book`, {
        method: "POST",
          ...
          <form className="booking-form" onSubmit={handleSubmit}>
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <input type="tel" name="phone" placeholder="+94 7X XXX XXXX" value={formData.phone} onChange={handleChange} required />
              </div>
            </div>
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <input type="number" name="travellers" min="1" placeholder="1" value={formData.travellers} onChange={handleChange} required />
              </div>
            </div>

            <div className="form-group">
              <label>Additional Notes (Optional)</label>
              <textarea name="note" rows="4" placeholder="Any special requests or comments?" value={formData.note} onChange={handleChange} />
            </div>

            <div className="popup-buttons">
              <button type="submit" className="btn-primary" disabled={loading}>
                {loading ? (<><i className="fas fa-spinner fa-spin"></i> Sending...</>) : (<><i className="fas fa-paper-plane"></i> Submit Inquiry</>)}
              </button>
...
const Tours = () => {
  ...
  const openSummary = (pkg) => { setSelectedPackage(pkg); setShowPackagePopup(true); };
  const closeSummary = () => { setSelectedPackage(null); setShowPackagePopup(false); };
  const receiveNow = () => { setShowPackagePopup(false); setShowBookingForm(true); };
  const closeBookingForm = () => setShowBookingForm(false);
  const onBooked = () => { setShowBookingForm(false); setShowConfirmation(true); };
